...
procedure Send_Mail(psTo IN VARCHAR2, psSubj IN VARCHAR2, psMsg IN VARCHAR2,
psAttachmentName VARCHAR2 DEFAULT NULL,
pcAttachmentCLOB CLOB DEFAULT NULL) ;
END;
/
...
type recp_typ is table of varchar2(100) index by binary_integer;
recp_list  recp_typ;
cnt  number ;
sList VARCHAR2(1000) := psTo;
sAttachment   VARCHAR2(1000);
sText   VARCHAR2(4000);
nOffset number:=1;
nLength number:=0;
nRead   number:=4000;
nLength2    NUMBER:=0;
begin
c := utl_smtp.open_connection( '<< Insert value >>', 25 );
utl_smtp.helo( c, '<< Insert value >>' );
utl_smtp.mail( c, '<< Insert value >>' );
cnt := 0;
while instr(sList,';') > 0 loop
cnt := NVL(recp_list.LAST,0) + 1;
recp_list(cnt) := substr(sList,1, instr(sList,';') - 1);
sList := substr(sList, instr(sList,';') +1 , length(sList));
end loop;
cnt := NVL(recp_list.LAST,0) + 1;
recp_list(cnt) := sList;
for cnt in recp_list.first .. recp_list.last loop
utl_smtp.rcpt( c, recp_list(cnt));
end loop;
utl_smtp.open_data(c);
utl_smtp.write_data(c, 'Date'||':'||TO_CHAR( SYSDATE, 'dd Mon yy hh24:mi:ss' )|| utl_tcp.CRLF);
utl_smtp.write_data(c, 'From' || ': ' || 'Oracle Server Mail' || utl_tcp.CRLF);
utl_smtp.write_data(c, 'To' || ': ' || psTo || utl_tcp.CRLF);
utl_smtp.write_data(c, 'Subject' || ': ' || psSubj || utl_tcp.CRLF);
sAttachment:='Sample Attachment';
if psAttachmentName is null then
utl_smtp.write_data(c, utl_tcp.CRLF || psMsg);
else
utl_smtp.write_data(c, 'Content-Type: multipart/mixed;'|| utl_tcp.CRLF);
utl_smtp.write_data(c, ' boundary="-----SECBOUND"'|| utl_tcp.CRLF);
utl_smtp.write_data(c, utl_tcp.CRLF);
utl_smtp.write_data(c, '-------SECBOUND'|| utl_tcp.CRLF);
...
utl_smtp.write_data(c, utl_tcp.CRLF);
utl_smtp.write_data(c, psMsg);
utl_smtp.write_data(c, utl_tcp.CRLF);
utl_smtp.write_data(c, '-------SECBOUND'|| utl_tcp.CRLF);
utl_smtp.write_data(c, ' name="'||psAttachmentName||'"'|| utl_tcp.CRLF);
utl_smtp.write_data(c, 'Content-Transfer_Encoding: 8bit'|| utl_tcp.CRLF);
utl_smtp.write_data(c, 'Content-Disposition: attachment;'|| utl_tcp.CRLF);
utl_smtp.write_data(c, ' filename="'||psAttachmentName||'"'|| utl_tcp.CRLF);
utl_smtp.write_data(c, utl_tcp.CRLF);
begin
LOOP
stext:='';
IF (dbms_lob.substr(pcAttachmentCLOB, 1, nOffset+nRead) = '.') THEN
nRead := nRead - 1;
END IF;
DBMS_LOB.READ(pcAttachmentCLOB,nRead,nOffset,stext);
nLength:=NVL(length(sText),0);
if nLength<=0 then
exit;
else
nOffset:=nOffset+nLength;
end if;
utl_smtp.write_data(c, sText);
END LOOP;
exception
